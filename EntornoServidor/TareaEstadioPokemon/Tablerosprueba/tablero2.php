...
<table>
    ...
    for ($i = 0; $i < 4; $i++) {
        ...
        //Primero ponemos la iamgen del pokemon
        echo "<td><img src='".$pokemonSeleccionado['img']."'></td>";
        //hacemos un foreach para sacar el valor de las keys.
        foreach ($pokemonSeleccionado as $key => $value) {
            //Si no es la imagen, mostramos el valor
            if ($key != 'img') {
                //Usamos ucfirst para que la primera letra sea mayuscula
                echo "<td>". ucfirst($key)." : ".ucfirst($value)."</td>";
            }
        }
